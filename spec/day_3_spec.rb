...
describe 'day 3' do
  ...
  describe 'examples' do
    class Example
      attr_reader :input, :expected_steps

      def initialize(input, expected_steps)
        @input = input
    ...
    examples.each do |example|
      it "should be able to calculate #{example.input}" do
        result = calculate_minimum_amount_of_steps(example.input)
        expect(result).to be example.expected_steps
      end
    end
  end
end
